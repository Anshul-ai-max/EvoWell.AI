...
interface Exercise { name: string; sets: number; reps: number; rest: string; tips: string; }
interface WorkoutDay { day: string; focus: string; exercises: Exercise[]; }
interface ExerciseState extends Exercise { id: string; completed: boolean; }
...
export default function Workout() {
  ...
  useEffect(() => {
    ...
    if (!day) return;
    setExercises(
      (day.exercises || []).map((ex, i) => ({ ...ex, id: `${selectedDay}-${i}`, completed: false }))
    );
    setExpandedId(null);
  }, [selectedDay, plan]);
  ...
  const handleCustomize = async () => {
    ...
    try {
      const { data: newPlan, error } = await supabase.functions.invoke('modify-plan', {
        body: { currentPlan: { days: plan }, modifyRequest: customizeText, planType: "workout" },
      });
  ...
  return (
    <div className="space-y-6 pb-6">
      ...
      {/* Day selector */}
      <motion.div className="flex gap-2 overflow-x-auto pb-1" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
        {plan.map((d, i) => {
          ...
          return (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className={cn(
                "flex h-14 min-w-[3.5rem] shrink-0 flex-col items-center justify-center rounded-2xl text-xs font-medium transition-all",
                isActive
      ...
      {exercises.length === 0 ? (
        ...
        <>
          ...
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            <Card className={cn("border-0 shadow-sm overflow-hidden rounded-2xl", progressPct === 100 && "ring-2 ring-warning/30")}>
          ...
          <div className="space-y-3">
            {exercises.map((ex, i) => {
              ...
              return (
                <motion.div key={ex.id} custom={i} variants={cardVariants} initial="hidden" animate="visible">
                  <Card className={cn(
                    "border-0 shadow-sm transition-all overflow-hidden rounded-2xl",
                    ex.completed ? "opacity-60" : "hover-lift",
                    !ex.completed && "border-l-4 border-l-primary/30"
                  )}>
                      ...
                      <div className="flex items-center gap-3 p-4">
                        ...
                        <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setExpandedId(isExpanded ? null : ex.id)}>
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            <Badge variant="secondary" className="text-[10px] px-1.5 py-0 h-5 rounded-lg">
                              <Zap className="h-3 w-3 mr-0.5" />{ex.sets}×{ex.reps}
                            </Badge>
                            <Badge variant="secondary" className="text-[10px] px-1.5 py-0 h-5 rounded-lg">
                              <Clock className="h-3 w-3 mr-0.5" />{ex.rest}
                            </Badge>
                          </div>
                        </div>
