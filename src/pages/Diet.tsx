...
import { UtensilsCrossed, Info, Pill, Sunrise, Sun, Moon, Cookie, ChevronDown, Flame, Beef, Wheat, Droplets, Pencil, Loader2, WandSparkles } from "lucide-react";
...
interface Meal { name: string; time: string; items: string[]; calories: number; protein: number; carbs: number; fat: number; }
interface DietPlan { meals: Meal[]; proteinNote?: string; supplementSuggestions?: string[]; dailyTarget?: number; }

const mealIconMap: Record<string, typeof Sunrise> = {
  breakfast: Sunrise, lunch: Sun, dinner: Moon, snack: Cookie, "pre-workout": Flame, "post-workout": Beef,
};

const mealColorMap: Record<string, string> = {
  breakfast: "border-l-warning", lunch: "border-l-info", dinner: "border-l-primary", snack: "border-l-success",
  "pre-workout": "border-l-destructive", "post-workout": "border-l-success",
};

function getMealIcon(name: string) {
  const lower = name.toLowerCase();
  for (const key of Object.keys(mealIconMap)) { if (lower.includes(key)) return mealIconMap[key]; }
  return UtensilsCrossed;
}

function getMealColor(name: string) {
  const lower = name.toLowerCase();
  for (const key of Object.keys(mealColorMap)) { if (lower.includes(key)) return mealColorMap[key]; }
  ...
  return (
    <div className="relative flex items-center justify-center">
      ...
      <div className="absolute flex flex-col items-center">
        <motion.span className="font-display text-2xl font-bold text-foreground" initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.3, duration: 0.5 }}>
          {current}
        </motion.span>
...
function MacroBar({ label, value, max, color, icon: Icon, delay }: {
  label: string; value: number; max: number; color: string; icon: typeof Beef; delay: number;
}) {
  const pct = max > 0 ? Math.min((value / max) * 100, 100) : 0;
  return (
    <motion.div className="flex items-center gap-3" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay, duration: 0.4 }}>
...
export default function Diet() {
  ...
  useEffect(() => {
    ...
    if (raw) {
      try {
        ...
        setPlan({ meals: parsed.meals || [], proteinNote: parsed.proteinNote, supplementSuggestions: parsed.supplementSuggestions, dailyTarget: parsed.dailyTarget });
      } catch { /* ignore */ }
    }
  }, []);
  ...
  const handleCustomize = async () => {
    ...
    try {
      ...
      setPlan({ meals: newPlan.meals || [], proteinNote: newPlan.proteinNote, supplementSuggestions: newPlan.supplementSuggestions, dailyTarget: newPlan.dailyTarget });
    ...
    <div className="space-y-6 pb-6">
      ...
      {proteinNote && (
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
          <Card className="border-0 shadow-sm overflow-hidden rounded-2xl">
            <div className="flex">
              <div className="w-1 gradient-primary shrink-0" />
            ...
            <CardContent className="p-4">
              ...
              <div className="flex flex-wrap gap-2">
                {supplementSuggestions.map((s, i) => (
                  <motion.div key={i} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 + i * 0.05 }}>
                    <Badge variant="secondary" className="px-3 py-1.5 text-xs font-medium rounded-lg">{s}</Badge>
                  </motion.div>
                ))}
              </div>
            </CardContent>
      ...
      <div className="space-y-3">
        {meals.map((meal, i) => {
          ...
          const mealBorderColor = getMealColor(meal.name);
          const isExpanded = expandedMeal === meal.name;

          return (
            <motion.div key={meal.name} custom={i} variants={cardVariants} initial="hidden" animate="visible">
              <Card
                className={`border-0 shadow-sm cursor-pointer overflow-hidden rounded-2xl hover-lift border-l-4 ${mealBorderColor}`}
                        ...
                        <div className="px-4 pb-4 border-t border-border">
                          <div className="pt-3 space-y-2">
                            {meal.items.map((item, idx) => (
                              <motion.div key={item} className="flex items-start gap-2.5 text-sm" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.04 }}>
                          ...
                          <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-border">
                            <Badge className="bg-success/10 text-success border-0 text-xs rounded-lg">P: {meal.protein}g</Badge>
                            <Badge className="bg-info/10 text-info border-0 text-xs rounded-lg">C: {meal.carbs}g</Badge>
                            <Badge className="bg-warning/10 text-warning border-0 text-xs rounded-lg">F: {meal.fat}g</Badge>
                          </div>
                        </div>
                      </motion.div>
                    )}
      ...
      <motion.div className="fixed bottom-24 right-4 z-40" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.5, type: "spring" }}>
        <Button size="lg" className="rounded-full shadow-lg gap-2 px-5 gradient-primary" onClick={() => setCustomizeOpen(true)}>
      ...
      <Sheet open={customizeOpen} onOpenChange={setCustomizeOpen}>
        <SheetContent side="bottom" className="rounded-t-3xl border-t border-border/60 bg-card/95 backdrop-blur-md">
          <SheetHeader>
            ...
            <SheetDescription>Describe what you want to change — e.g. "I can't eat soya or sprouts"</SheetDescription>
          </SheetHeader>
          <div className="mt-4 space-y-4 rounded-2xl border border-border/70 bg-muted/20 p-4">
            <label htmlFor="dietCustomize" className="block text-xs font-semibold uppercase tracking-wide text-muted-foreground">
              Diet change request
            </label>
            <div className="relative">
              <WandSparkles className="pointer-events-none absolute left-3 top-3.5 h-4 w-4 text-muted-foreground" />
              <Textarea id="dietCustomize" placeholder="e.g. Replace paneer with tofu, I'm lactose intolerant..." value={customizeText} onChange={(e) => setCustomizeText(e.target.value)} className="min-h-[120px] pl-10" />
            </div>
